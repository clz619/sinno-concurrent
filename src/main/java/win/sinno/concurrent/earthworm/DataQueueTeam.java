...
public class DataQueueTeam<DATA> implements Runnable {
  ...
  public DataQueueTeam(String name, DataQueueBoss<DATA> dataQueueBoss,
      IDataHandler<DATA> dataHandler) {
    ...
    this.executorService = new ThreadPoolExecutor(0, Integer.MAX_VALUE,
        ...
        new SynchronousQueue<Runnable>(), new CustomThreadFactory("dqt-" + name));

    //固定一个的线程池，维护future处理
    this.futureExecutorService = new ThreadPoolExecutor(1, 1,
        0L, TimeUnit.MILLISECONDS,
        new LinkedBlockingQueue<Runnable>(),
        new CustomThreadFactory("dqt-" + name + "-ft"));
  ...
  public DataQueueTeam(String name, DataQueueBoss<DATA> dataQueueBoss,
      IDataHandler<DATA> dataHandler, Boolean useTimeout, Long timeout) {
        ...
        if ((task = this.dataQueueBoss.getOneTask()) != null) {
          ...
          try {
            ...
            Future future = this.executorService
                .submit(new DataQueueWorker<DATA>(this.dataHandler, task));
  ...
  public String status() {
    ...
    status.append("\n createTs     ").append(dateFormat.format(new Date(createTs)));
    status.append("\n-----Status------------");
        ...
        if (future.isDone() || future.isCancelled()) {
          ...
          if (maxTs < System.currentTimeMillis()) {
            ...
            if (!(future.isDone() || future.isCancelled())) {
              ...
              //开始时间
              LOG.warn("{} beginTs:{} is time out.",
                  new Object[]{futureHandler.getOriginData().toString(),
                      futureHandler.getBeginTs()});
